	...
	if ( $settings['show_image'] ) :
		...
		<div class="premium-fb-rev-page-left">
			...
			if ( empty( $custom_image ) ) {
				...
				/**
				 * Performance Optimization: Check for already cached image URL from transient
				 * or fallback option. This avoids redundant external API calls and database
				 * writes on every widget render.
				 */
				if ( isset( $place->image_url ) && ! empty( $place->image_url ) ) {
					$place_img = $place->image_url;
				} else {
...
function premium_place_avatar_new( $place_data, $api_key ) {
	...
	/**
	 * Performance Optimization: Return cached image URL if available.
	 * This avoids re-resolving the avatar via an external HTTP request.
	 */
	if ( isset( $place_data->image_url ) ) {
 ...
 */
function premium_yelp_reviews_data( $api_key, $place_id, $lang = '' ) {

	$yelp_response = rplg_urlopen( premium_yelp_reviews_api( $place_id, $lang ), null, array( 'Authorization: Bearer ' . $api_key ) );
