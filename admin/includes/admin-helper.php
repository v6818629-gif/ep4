...
class Admin_Helper {
	...
	public function insert_action_links( $links ) {
		...
		if ( isset( $license_info['id'] ) && '4' !== $license_info['id'] ) {
			...
			$upgrade_link = sprintf( '<a href="%s" title="Upgrade to Lifetime by paying the difference only with an additional 20%% OFF" target="_blank" style="color: #FF6000; font-weight: bold;">%s</a>', $link, __( 'Upgrade to Lifetime (20% OFF)', 'premium-addons-pro' ) );
	...
	public function action_papro_license_activate() {
		...
		$license = isset( $_POST['papro_license_key'] ) ? trim( $_POST['papro_license_key'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
	...
	public function save_white_labeling_settings() {
		...
		$new_settings = array(
			...
			'premium-wht-lbl-row'             => isset( $settings['premium-wht-lbl-row'] ) ? 1 : 0,
			'premium-wht-lbl-changelog'       => isset( $settings['premium-wht-lbl-changelog'] ) ? 1 : 0,
			'premium-wht-lbl-option'          => isset( $settings['premium-wht-lbl-option'] ) ? 1 : 0,
			'premium-wht-lbl-rate'            => isset( $settings['premium-wht-lbl-rate'] ) ? 1 : 0,
			'premium-wht-lbl-about'           => isset( $settings['premium-wht-lbl-about'] ) ? 1 : 0,
			'premium-wht-lbl-license'         => isset( $settings['premium-wht-lbl-license'] ) ? 1 : 0,
			'premium-wht-lbl-not'             => isset( $settings['premium-wht-lbl-not'] ) ? 1 : 0,
			'premium-wht-lbl-logo'            => isset( $settings['premium-wht-lbl-logo'] ) ? 1 : 0,
			'premium-wht-lbl-version'         => isset( $settings['premium-wht-lbl-version'] ) ? 1 : 0,
		);
